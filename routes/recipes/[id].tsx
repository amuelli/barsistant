...
import type { RecipeWithFullIngredients } from "../../utils/recipe-model.ts";
import { recipeModel } from "../../utils/recipe-model.ts";

interface RecipeData {
  recipe: RecipeWithFullIngredients;
}

export const handler: Handlers<RecipeData> = {
  async GET(_req, ctx) {
...
export default function RecipePage({ data }: PageProps<RecipeData>) {
  const { recipe } = data;
        ...
        <div>
          ...
          <ul class="space-y-2">
            {recipe.ingredients.map((ingredient) => (
              <li key={ingredient.ingredientId} class="flex items-center gap-2">
                <div class="flex-1">
